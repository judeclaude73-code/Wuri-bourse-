...
const MAX_PROOF_BASE64_LENGTH = 7_000_000; // ~5 Mo décodé
...
// Migration défensive : ajoute les colonnes si la base existait déjà sans elles
for (const col of ["proof_path TEXT", "proof_mime TEXT"]) {
  try {
...
const STARTING_CASH = 0; // un vrai utilisateur ne reçoit pas d'argent gratuit — il doit déposer
...
function requireAdmin(req, res) {
  ...
  if (!user.is_admin) {
    sendJson(res, 403, { error: "Accès réservé à un compte administrateur." });
    return null;
  }
  return user;
}
...
route("POST", "/api/register", async (req, res) => {
  ...
  const isFirstUser = userCount === 0; // le tout premier compte créé devient administrateur

  const pw = hashSecret(password);
  const pn = hashSecret(pin);
  const info = db
    .prepare(
      `INSERT INTO users (name, phone, password_hash, password_salt, pin_hash, pin_salt, cash, is_admin)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(name, phone, pw.hash, pw.salt, pn.hash, pn.salt, STARTING_CASH, isFirstUser ? 1 : 0);
...
route("POST", "/api/deposit/init", async (req, res) => {
  const user = getAuthUser(req);
  if (!user) return sendJson(res, 401, { error: "Non authentifié." });
  const { amount, methodId } = await readJsonBody(req);
  const method = methodById(methodId);
  if (!method || !amount || amount <= 0) return sendJson(res, 400, { error: "Requête invalide." });
  // Pas besoin du numéro du client pour déposer : on lui montre directement
  // le numéro configuré par l'administrateur, il envoie l'argent lui-même.
  ...
  const info = db
    .prepare(
      `INSERT INTO transactions (user_id, kind, method, amount, status, reference)
...
// ---- Preuve de paiement (capture d'écran jointe par le client) ----
const MIME_EXTENSIONS = { "image/png": "png", "image/jpeg": "jpg", "image/jpg": "jpg", "image/webp": "webp" };

route("POST", "/api/deposit/proof", async (req, res) => {
  const user = getAuthUser(req);
  if (!user) return sendJson(res, 401, { error: "Non authentifié." });
  const { transactionId, imageBase64, mimeType } = await readJsonBody(req);

  if (!transactionId || !imageBase64) return sendJson(res, 400, { error: "Image et identifiant de transaction requis." });
  if (imageBase64.length > MAX_PROOF_BASE64_LENGTH) return sendJson(res, 400, { error: "Image trop lourde (5 Mo maximum)." });
  const ext = MIME_EXTENSIONS[mimeType];
  if (!ext) return sendJson(res, 400, { error: "Format d'image non supporté (PNG, JPEG ou WEBP uniquement)." });

  const tx = db.prepare("SELECT * FROM transactions WHERE id = ? AND user_id = ? AND kind = 'depot'").get(transactionId, user.id);
  if (!tx) return sendJson(res, 404, { error: "Transaction introuvable." });
  if (tx.status !== "pending") return sendJson(res, 400, { error: "Cette transaction n'attend plus de preuve." });
  ...
  const filename = `proof_${transactionId}_${Date.now()}.${ext}`;
  fs.writeFileSync(path.join(UPLOADS_DIR, filename), buffer);

  // Supprime l'ancienne preuve si le client en avait déjà envoyé une
  if (tx.proof_path) {
    try { fs.unlinkSync(path.join(UPLOADS_DIR, tx.proof_path)); } catch {}
  }

  db.prepare("UPDATE transactions SET proof_path = ?, proof_mime = ? WHERE id = ?").run(filename, mimeType, transactionId);
  sendJson(res, 200, { ok: true });
});

// ---- Back-office admin : dépôts en attente, avec preuve si disponible ----
route("GET", "/api/admin/deposits/pending", async (req, res) => {
  const admin = requireAdmin(req, res);
  if (!admin) return;
  const rows = db
    .prepare(
      `SELECT t.id, t.amount, t.method, t.reference, t.created_at, t.proof_path,
              u.name AS userName, u.phone AS userPhone
       FROM transactions t
       JOIN users u ON u.id = t.user_id
       WHERE t.kind = 'depot' AND t.status = 'pending'
       ORDER BY t.id DESC`
    )
    .all();
  sendJson(res, 200, { deposits: rows.map((r) => ({ ...r, hasProof: !!r.proof_path, proof_path: undefined })) });
});

route("GET", "/api/admin/deposit-proof/:transactionId", async (req, res) => {
  const admin = requireAdmin(req, res);
  if (!admin) return;
  const tx = db.prepare("SELECT proof_path, proof_mime FROM transactions WHERE id = ?").get(req.params.transactionId);
  if (!tx || !tx.proof_path) return sendJson(res, 404, { error: "Aucune preuve pour cette transaction." });
  const filePath = path.join(UPLOADS_DIR, tx.proof_path);
  if (!fs.existsSync(filePath)) return sendJson(res, 404, { error: "Fichier introuvable sur le serveur." });
...
route("POST", "/api/deposit/confirm", async (req, res) => {
  // Réservé à un compte admin pour l'instant. En vraie production, cette route doit
  // être remplacée par un webhook signé du fournisseur de paiement (Orange, MTN,
  // CinetPay...) — même un admin humain ne devrait pas la déclencher à la main.
...
route("POST", "/api/withdraw/confirm", async (req, res) => {
  // Action admin/back-office (confirme que l'argent a bien été envoyé au client)
  if (!requireAdmin(req, res)) return;
  const { transactionId } = await readJsonBody(req);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "OPTIONS") {
    ...
    return res.end(); // un 204 ne doit contenir aucun corps de réponse
  }
